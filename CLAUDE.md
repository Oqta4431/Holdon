...
構造（フォームの組み立て・ルーティング・データの受け渡し）はユーザーが実装し、見た目（Tailwind CSS・daisyUI によるスタイリング）はあなたが実装する。

#### 進め方

1. **構造の実装**：`form_with` の組み立て、ループ、条件分岐など、Rails の概念が絡む部分はユーザーが実装する。レビュー・フィードバックは機能実装と同じ方針で行う。
2. **スタイリングの実装**：構造が固まったら、あなたが Tailwind CSS・daisyUI を使って見た目を実装する。既存画面のデザインパターンを参考にして統一感を保つこと。

#### 禁止事項

- 構造部分をユーザーに確認せずにスタイリングを実装すること
...
1. **コミット提案**：`git status` で未コミットの変更を確認し、論理的なまとまりでグループ化してコミット粒度とコミットメッセージを提案する。各コミットに適切なプレフィックスを付ける（`add:` / `change:` / `fix:` / `delete:` など）。ステージング、コミットなどの実際の操作はユーザーが行う。
